    ...
    for (char tmp_y = '1'; tmp_y <= '8'; ++tmp_y) {
      for (char tmp_x = 'a'; tmp_x <= 'h'; ++tmp_x) {
        if (mover->isPossible(tmp_x, tmp_y)) {
          ...
          mover->square =
              this->getMatrix().at(std::pair<char, char>(tmp_x, tmp_y));
          ...
          if (!isCheck(col, std::pair<char, char>('0', '0'))) {
            ...
            mover->square =
                this->getMatrix().at(std::pair<char, char>(src_x, src_y));
            mover->square->setOccupator(mover);
            break;
          }
          mover->square->setOccupator(was_captured);
...
bool Board::move(std::string instruction) {
  ...
  if (candidate_to_move->move(dest_x, dest_y, add_opt)) {
    std::cout << "Figure moved to: "
              << candidate_to_move->getSquare()->getCoords().first
              << candidate_to_move->getSquare()->getCoords().second
              << std::endl;
  ...
  // if there is still check after the "possible" move (Piece::move() returned
  // false, but it changed the board)
  if (undo_flag)
    undo();
  return false;
}
...
bool Board::isCheck(color col, std::pair<char, char> king_pos) {
   ...
   * @param king_pos particular position of king to check (useful f.e. while
...
void Board::deepCopy(Board *src) {
  ...
  for (auto &white_piece : white) {
    Square *my_square =
        this->matrix.at((white_piece->getSquare()->getCoords()));
  ...
  for (auto &black_piece : black) {
    Square *my_square =
        this->matrix.at((black_piece->getSquare()->getCoords()));
