...
std::string prepareInfoToFront(Board &chess_board, std::string &recived_move);

void showBoard(Board *chessBoard) {

    std::map<std::pair<char, char>, Square *, MyComparator> mt =
            chessBoard->getMatrix();
    ...
    for (char a = '8'; a >= '1'; a--) {
        ...
        for (char b = 'a'; b <= 'h'; b++) {
            if (mt.at(std::pair<char, char>(b, a))->getOccupator() == nullptr) {
                ...
                if (mt.at(std::pair<char, char>(b, a))->getOccupator()->getColor() ==
                    WHITE) {
                    std::cout
...
int main(int argc, char **argv) {
    ...
    servaddr.sin6_port = htons(my_port); /* daytime server */
    ...
    if (bind(listenfd, (struct sockaddr *) &servaddr, sizeof(servaddr)) < 0) {
        fprintf(stderr, "chess bind error : %s\n", strerror(errno));
        return 1;
    }
    ...
    for (;;) {
        ...
        inet_ntop(AF_INET6, (struct sockaddr *) &cliaddr.sin6_addr, str,
                  sizeof(str));
        ...
        std::string info_to_front = prepareInfoToFront(chess_board, recived_move);

        snprintf(buff, sizeof(buff), "%s",
                 info_to_front.c_str()); // there we would send response from
...
// COLOR TO MOVE
    ...
    std::string pieceTable[] = {"Pawn", "Rook", "Knight",
                                "Bishop", "Queen", "King"};
    std::string whiteCodeTable[] = {"#9817", "#9814", "#9816",
                                    "#9815", "#9813", "#9812"};
    std::string blackCodeTable[] = {"#9823", "#9820", "#9822",
                                    "#9821", "#9819", "#9818"};
    ...
    for (auto sq : squares) {
        ...
        if (sq.second->getOccupator() == nullptr) {
            ...
            info_to_front +=
                    colorTable[sq.second->getOccupator()->getColor()] + ",";
        ...
        if (sq.second->getOccupator() == nullptr) {
            ...
            if (sq.second->getOccupator()->getColor() == WHITE)
                info_to_front +=
                        whiteCodeTable[sq.second->getOccupator()->getType()] + ";";
            else
                info_to_front +=
                        blackCodeTable[sq.second->getOccupator()->getType()] + ";";
        }
    }
    info_to_front = info_to_front.substr(0, info_to_front.size() - 1);
    // std::cout << info_to_front << std::endl;
    return info_to_front;
}
